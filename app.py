...
# Intercettore globale anti-schermata nera/generica
try:
    import os
    import hmac
        ...
        with col_center:
            ...
            if st.button("SBLOCCA TERMINALE", use_container_width=True):
                try:
                    ...
                    st.error(f"Errore di autenticazione: {str(ex)}")
        st.stop()
    ...
    # -----------------------------------------------------------------------------
    # 4. ROUTER DINAMICO ISOLATO CON LOG DI SISTEMA
    # -----------------------------------------------------------------------------
    if nav == "1. Macro Intelligence & Fed Liquidity":
        try:
            ...
            st.error("ERRORE CRITICO NEL MODULO 1:")
        ...
        try:
            ...
            st.error("ERRORE CRITICO NEL MODULO 2:")
            st.code(traceback.format_exc())
    ...
    st.error("ERRORE CRITICO GLOBALE DI AVVIO DELL'APPLICAZIONE:")
    st.code(traceback.format_exc())
