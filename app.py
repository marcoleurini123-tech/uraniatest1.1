...
# -----------------------------------------------------------------------------
# 2. CREDENZIALI & NOTIFIER TELEGRAM (SOLO PER STOCK SCREENER)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3. GESTIONE AUTENTICAZIONE E SCHERMATA LOGIN
# -----------------------------------------------------------------------------
...
if not st.session_state.authenticated:
    ...
    # Risoluzione percorso immagine robusta
    logo_path = None
...
# -----------------------------------------------------------------------------
# 4. DATABASE LOCALE EOD & FETCHERS
# -----------------------------------------------------------------------------
...
def fetch_yahoo_eod(days=60):
    tickers = {"VIX9D": "^VIX9D", "VIX": "^VIX", "VIX3M": "^VIX3M", "VIX6M": "^VIX6M", 
...
# -----------------------------------------------------------------------------
# 5. ENGINE MATEMATICO (POC & SCREENER)
# -----------------------------------------------------------------------------
...
def evaluate_macro_visual_alerts(df: pd.DataFrame) -> list[dict]:
    ...
    # 1. Divergenza Liquidità Fed vs SPY
    if prev5.get('Net_Liquidity', 0) > 0:
        ...
        if liq_delta < -1.0 and last.get('SPY', 0) > prev5.get('SPY', 0):
            alerts.append({
                ...
                "severity": "CRITICAL",
                "color": "#ef4444",
                "desc": f"Liquidità Netta in calo ({liq_delta:+.2f}% a 5gg) mentre lo SPY sale. Rischio elevato di bull-trap o storno monetario."
            })
    ...
    if vix1d > vix and vix > 0:
        alerts.append({
            ...
            "severity": "HIGH",
            "color": "#f59e0b",
            "desc": f"Struttura a termine invertita ({vix1d:.1f} vs {vix:.1f}). Forte domanda di coperture immediate."
        })
    ...
    if skew >= 140 or move >= 115:
        alerts.append({
            "type": "Eccesso Rischio Coda (SKEW / MOVE)",
            "severity": "WARNING",
            "color": "#d97706",
            "desc": f"SKEW Index a {skew:.1f} e MOVE a {move:.1f}. Elevata domanda istituzionale di opzioni put OTM / tassi volatili."
        })

    return alerts

# -----------------------------------------------------------------------------
# 6. STYLING CSS DASHBOARD
# -----------------------------------------------------------------------------
st.markdown(
    """
    <style>
...
with st.sidebar:
    ...
    nav = st.radio(
        ...
        [
            "1. Panoramica Macro & Alert Visivi",
...
# ==============================================================================
# MODULO 1: PANORAMICA MACRO & ALERT VISIVI A VIDEO (NO TELEGRAM)
# ==============================================================================
if nav == "1. Panoramica Macro & Alert Visivi":
    st.title("Panoramica Macro e Mercati")
    st.caption("Intelligence quantitativa EOD su regimi macro, flussi di sentiment e monitoraggio divergenze a video.")

    if st.button("🔄 SINCRONIZZA FLUSSI EOD (DATABASE)"):
        with st.spinner("Aggiornamento database EOD in corso..."):
            d_y, d_b = fetch_yahoo_eod(60), fetch_bridge_data()
            try:
    ...
    # Visualizzazione Banner Alert Macro (Solo a Video)
    if not df.empty:
